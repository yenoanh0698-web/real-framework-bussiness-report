...
const mockHeroFull = [
  ...
  { id: 'M09', kpi: 105, scoreE: 1.54, risk: 'Chưa kiểm chứng tiến bộ sau coaching' },
  { id: 'M11', kpi: 104, scoreE: 1.92, risk: 'Chưa xây dựng nguồn kế thừa' },
  { id: 'M15', kpi: 103, scoreE: 1.67, risk: 'Quyền quyết định tập trung vào quản lý' },
  { id: 'M20', kpi: 101, scoreE: 1.75, risk: 'Chưa giao nhiệm vụ phát triển có cấu trúc' },
  { id: 'M24', kpi: 100, scoreE: 1.88, risk: 'Ý kiến nhân viên chưa được phản hồi rõ' },
  { id: 'M29', kpi: 100, scoreE: 1.58, risk: 'Kiểm tra liên tục và sửa trực tiếp' }
];
...
const App = () => {
        ...
        <div style={{ height: 260 }}>
          <ResponsiveContainer>
            <BarChart data={mockBarHours} layout="vertical" margin={{ top: 5, right: 60, left: 30, bottom: 5 }}>
              ...
              <YAxis dataKey="name" type="category" width={180} tick={{ fontSize: 13, fill: '#4B5563', fontWeight: 600 }} axisLine={false} tickLine={false} />
  ...
  const Tab5 = () => (
    <div className="flex-col gap-4">
      <h2 className="section-title">Phần 3: Phân tích Nhóm Hero vs Nhóm Hệ thống</h2>
      {/* Section 2 (Brought to top) */}
      <div className="grid-2 gap-4">
        <div className="card flex-center position-relative">
          <div style={{ height: 250, width: '100%' }}>
            <ResponsiveContainer>
              <PieChart>
                <Pie data={[
                  { name: 'Quản lý Hệ thống', value: 5, fill: COLORS.excellent },
                  { name: 'Hero cá nhân', value: 9, fill: COLORS.urgent },
                  { name: 'Có nền tảng', value: 3, fill: COLORS.standard },
                  { name: 'Cần can thiệp', value: 13, fill: COLORS.significant }
        ...
        <div className="grid-2 gap-4">
          <div className="card flex-col flex-center" style={{ borderLeft: `4px solid ${COLORS.excellent}` }}>
            ...
            <h4 style={{ margin: 0, fontSize: '1.1rem' }}>Quản lý Hệ thống</h4>
            <div style={{ fontSize: '1.5rem', fontWeight: 'bold', marginTop: '0.25rem' }}>5 người</div>
          </div>
          <div className="card flex-col flex-center" style={{ borderLeft: `4px solid ${COLORS.urgent}` }}>
            ...
            <h4 style={{ margin: 0, fontSize: '1.1rem' }}>Hero cá nhân</h4>
            <div style={{ fontSize: '1.5rem', fontWeight: 'bold', marginTop: '0.25rem' }}>9 người</div>
          </div>
          <div className="card flex-col flex-center" style={{ borderLeft: `4px solid ${COLORS.significant}` }}>
            ...
            <h4 style={{ margin: 0, fontSize: '1.1rem' }}>Cần can thiệp</h4>
            <div style={{ fontSize: '1.5rem', fontWeight: 'bold', marginTop: '0.25rem' }}>13 người</div>
          </div>
          <div className="card flex-col flex-center" style={{ borderLeft: `4px solid ${COLORS.standard}` }}>
            ...
            <h4 style={{ margin: 0, fontSize: '1.1rem' }}>Có nền tảng</h4>
            <div style={{ fontSize: '1.5rem', fontWeight: 'bold', marginTop: '0.25rem' }}>3 người</div>
          </div>
        </div>
      </div>
      ...
      <div className="card">
        <h3 className="card-title" style={{ marginBottom: '1rem' }}>Phân tích Nhóm Hero vs Nhóm Hệ thống (Góc nhìn từ trụ cột E - Empowerment)</h3>
        <div style={{ marginBottom: '1.5rem', padding: '0.75rem 1rem', backgroundColor: '#F3F4F6', borderRadius: '8px', fontSize: '0.9rem', color: '#374151' }}>
          <strong>Ngưỡng phân loại giả định:</strong> KPI đạt: từ 100% | E đạt chuẩn: từ 2.00
        </div>
        
        <div className="grid-2 gap-4">
          <div style={{ height: 350, position: 'relative', marginBottom: '0.5rem' }}>
            <ResponsiveContainer>
              <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
                ...
                <ReferenceLine x={2.0} stroke="#999" strokeDasharray="5 5" />
                <ReferenceLine y={100} stroke="#999" strokeDasharray="5 5" />
            ...
            <div style={{ position: 'absolute', top: 10, left: 70, color: COLORS.urgent, fontWeight: 'bold', backgroundColor: 'rgba(255,255,255,0.8)', padding: '2px 4px', borderRadius: '4px', fontSize: '0.85rem' }}>Hero cá nhân (9)</div>
            <div style={{ position: 'absolute', top: 10, right: 30, color: COLORS.excellent, fontWeight: 'bold', backgroundColor: 'rgba(255,255,255,0.8)', padding: '2px 4px', borderRadius: '4px', fontSize: '0.85rem' }}>Quản lý Hệ thống (5)</div>
            <div style={{ position: 'absolute', bottom: 50, left: 70, color: COLORS.significant, fontWeight: 'bold', backgroundColor: 'rgba(255,255,255,0.8)', padding: '2px 4px', borderRadius: '4px', fontSize: '0.85rem' }}>Cần can thiệp (13)</div>
            <div style={{ position: 'absolute', bottom: 50, right: 30, color: COLORS.standard, fontWeight: 'bold', backgroundColor: 'rgba(255,255,255,0.8)', padding: '2px 4px', borderRadius: '4px', fontSize: '0.85rem' }}>Có nền tảng (3)</div>
          </div>
          
          <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
            <div style={{ padding: '1.25rem', backgroundColor: '#FEF2F2', borderRadius: '8px', borderLeft: '4px solid #991B1B' }}>
              <strong style={{ color: '#991B1B', fontSize: '1.1rem' }}>📌 Kết luận:</strong> 
              <p style={{ color: '#7F1D1D', marginTop: '0.75rem', marginBottom: '0.5rem', lineHeight: '1.5' }}>
                Có <strong>9 Hero cá nhân</strong> (KPI cao nhưng Điểm E thấp). Đây chính là nhóm gây ra phần lớn trong <strong>214 giờ lãng phí</strong> đã phân tích ở P2.
              </p>
              <p style={{ color: '#7F1D1D', marginBottom: '0.5rem', lineHeight: '1.5' }}>
                Họ đạt KPI nhưng lại duy trì kết quả thông qua 3 hành vi yếu kém thuộc trụ cột Empowerment:
              </p>
              <ul style={{ margin: 0, paddingLeft: '1.5rem', color: '#7F1D1D', lineHeight: '1.6' }}>
                <li>Can thiệp quá sâu vào công việc của nhân viên (Làm thay).</li>
                <li>Trực tiếp giải quyết vấn đề thay vì hướng dẫn.</li>
                <li>Không duy trì được kết quả khi vắng mặt.</li>
              </ul>
            </div>
          </div>
        </div>
      ...
      {/* Section 3: Định hướng Giải pháp & Khuyến nghị */}
      <div className="card" style={{ marginBottom: '1.5rem' }}>
        <h3 className="card-title" style={{ marginBottom: '1.5rem' }}>Định hướng Giải pháp & Khuyến nghị</h3>
        
        {/* Principles Banner (Full Width) */}
        <div style={{ backgroundColor: '#1E3A8A', borderRadius: '12px', overflow: 'hidden', display: 'flex', flexDirection: 'column', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1)', marginBottom: '1.5rem' }}>
          <div style={{ backgroundColor: '#172554', padding: '1rem 1.5rem', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
            <i className="fas fa-shield-alt" style={{ color: '#60A5FA', fontSize: '1.25rem' }}></i>
            <strong style={{ color: 'white', fontSize: '1.1rem', letterSpacing: '0.5px' }}>NGUYÊN TẮC SỬ DỤNG KẾT QUẢ</strong>
          </div>
          <div style={{ padding: '1.25rem 1.5rem', color: 'white' }}>
            <p style={{ margin: '0 0 1rem 0', fontSize: '0.95rem', color: '#BFDBFE' }}>
              Không sử dụng điểm test như căn cứ duy nhất để bổ nhiệm, kỷ luật hoặc loại bỏ quản lý. Kết quả cần được đối chiếu với:
            </p>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1rem 2rem' }}>
              <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                <i className="fas fa-check-circle" style={{ color: '#3B82F6', fontSize: '1rem' }}></i>
                <span style={{ fontSize: '0.9rem' }}>Đánh giá KPI thực tế</span>
              </div>
              <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                <i className="fas fa-check-circle" style={{ color: '#3B82F6', fontSize: '1rem' }}></i>
                <span style={{ fontSize: '0.9rem' }}>Bằng chứng hành vi</span>
              </div>
              <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                <i className="fas fa-check-circle" style={{ color: '#3B82F6', fontSize: '1rem' }}></i>
                <span style={{ fontSize: '0.9rem' }}>Yêu cầu của vai trò</span>
              </div>
              <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                <i className="fas fa-check-circle" style={{ color: '#3B82F6', fontSize: '1rem' }}></i>
                <span style={{ fontSize: '0.9rem' }}>Dữ liệu từ cấp trên và đội ngũ</span>
              </div>
              <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                <i className="fas fa-check-circle" style={{ color: '#3B82F6', fontSize: '1rem' }}></i>
                <span style={{ fontSize: '0.9rem' }}>Tiến bộ sau 90 ngày</span>
              </div>
            </div>
          </div>
        </div>

        {/* 4 Groups Recommendations (2x2 Grid) */}
        <div className="grid-2 gap-4">
            {/* Priority 1: Hero */}
            <div style={{ padding: '1.25rem', border: `1px solid ${COLORS.urgent}`, borderRadius: '12px', backgroundColor: '#FEF2F2', boxShadow: '0 2px 4px rgba(0,0,0,0.02)' }}>
              <h4 style={{ color: COLORS.urgent, margin: '0 0 0.75rem 0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <i className="fas fa-bomb" style={{ fontSize: '1.1rem' }}></i>
                  <span style={{ fontSize: '1.1rem' }}>Hero cá nhân</span>
                </div>
                <span style={{ backgroundColor: COLORS.urgent, color: 'white', padding: '4px 12px', borderRadius: '16px', fontSize: '0.85rem', fontWeight: 'bold' }}>Ưu tiên (9)</span>
              </h4>
              <p style={{ margin: 0, fontSize: '0.95rem', color: '#7F1D1D', lineHeight: '1.6' }}>
                Chưa sử dụng KPI làm căn cứ thăng chức duy nhất; thực hiện kế hoạch E trong 90 ngày.
              </p>
            </div>
            
            {/* Priority 2: Cần can thiệp */}
            <div style={{ padding: '1.25rem', border: `1px solid ${COLORS.significant}`, borderRadius: '12px', backgroundColor: '#FFF7ED', boxShadow: '0 2px 4px rgba(0,0,0,0.02)' }}>
              <h4 style={{ color: COLORS.significant, margin: '0 0 0.75rem 0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <i className="fas fa-wrench" style={{ fontSize: '1.1rem' }}></i>
                  <span style={{ fontSize: '1.1rem' }}>Cần can thiệp</span>
                </div>
                <span style={{ backgroundColor: COLORS.significant, color: 'white', padding: '4px 12px', borderRadius: '16px', fontSize: '0.85rem', fontWeight: 'bold' }}>Cải thiện (13)</span>
              </h4>
              <p style={{ margin: 0, fontSize: '0.95rem', color: '#9A3412', lineHeight: '1.6' }}>
                Phát triển có cấu trúc; cấp trên quan sát; đánh giá lại sau 90 ngày.
              </p>
            </div>

            {/* Priority 3: Hệ thống */}
            <div style={{ padding: '1.25rem', border: `1px solid ${COLORS.excellent}`, borderRadius: '12px', backgroundColor: '#F0FDF4', boxShadow: '0 2px 4px rgba(0,0,0,0.02)' }}>
              <h4 style={{ color: COLORS.excellent, margin: '0 0 0.75rem 0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <i className="fas fa-star" style={{ fontSize: '1.1rem' }}></i>
                  <span style={{ fontSize: '1.1rem' }}>Quản lý Hệ thống</span>
                </div>
                <span style={{ backgroundColor: COLORS.excellent, color: 'white', padding: '4px 12px', borderRadius: '16px', fontSize: '0.85rem', fontWeight: 'bold' }}>Nhân bản (5)</span>
              </h4>
              <p style={{ margin: 0, fontSize: '0.95rem', color: '#166534', lineHeight: '1.6' }}>
                Đưa vào nguồn kế thừa; giao vai trò mentor; phát triển hướng Mastery.
              </p>
            </div>
            ...
            <div style={{ padding: '1.25rem', border: `1px solid ${COLORS.standard}`, borderRadius: '12px', backgroundColor: '#F3F4F6', boxShadow: '0 2px 4px rgba(0,0,0,0.02)' }}>
              <h4 style={{ color: COLORS.standard, margin: '0 0 0.75rem 0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  ...
                  <span style={{ fontSize: '1.1rem' }}>Có nền tảng</span>
                </div>
      ...
      {/* Section 4: Danh sách cảnh báo "Hero" cá nhân (Moved DOWN to be the final action) */}
      <div className="card">
        <h3 className="card-title">Danh sách cảnh báo "Hero" cá nhân</h3>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.9rem' }}>
          <thead>
            <tr style={{ backgroundColor: '#F9FAFB', borderBottom: '2px solid #E5E7EB' }}>
              <th style={{ padding: '0.75rem', textAlign: 'left' }}>Mã QL</th>
              <th style={{ padding: '0.75rem', textAlign: 'center' }}>KPI 6 tháng</th>
              <th style={{ padding: '0.75rem', textAlign: 'center' }}>Điểm E</th>
              <th style={{ padding: '0.75rem', textAlign: 'left' }}>Hành vi rủi ro nổi bật</th>
            </tr>
          </thead>
          <tbody>
        ...
        <div style={{ marginTop: '1.5rem', padding: '1.25rem', backgroundColor: '#FEF9C3', border: '1px solid #FDE047', borderRadius: '8px' }}>
          <strong style={{ color: '#854D0E', fontSize: '1.05rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            <span>⚡</span> Hành động bắt buộc
          </strong>
          <ul style={{ marginTop: '0.75rem', marginBottom: 0, paddingLeft: '1.5rem', color: '#713F12', lineHeight: '1.6' }}>
            <li>Mỗi quản lý chọn hai hành vi E ưu tiên.</li>
            <li>Thực hiện tối thiểu ba tình huống công việc có quan sát.</li>
            <li>Đánh giá lại sau 90 ngày.</li>
            <li>Chỉ xác nhận sự tiến bộ khi có bằng chứng hành vi, không chỉ dựa trên việc hoàn thành khóa học.</li>
          </ul>
        </div>
      </div>
    </div>
